...
![Build Status](https://img.shields.io/github/workflow/status/jon-hedgerows/traefik-forward-auth/CI) [![Go Report Card](https://goreportcard.com/badge/github.com/jon-hedgerows/traefik-forward-auth)](https://goreportcard.com/report/github.com/jon-hedgerows/traefik-forward-auth) ![Github Downloads](https://img.shields.io/github/downloads/jon-hedgerows/traefik-forward-auth/total) [![GitHub release](https://img.shields.io/github/release/jon-hedgerows/traefik-forward-auth.svg)](https://GitHub.com/jon-hedgerows/traefik-forward-auth/releases/)
...
We recommend using the `2` tag on docker hub (`jon-hedgerows/traefik-forward-auth:2`).

You can also use the latest incremental releases found on [docker hub](https://hub.docker.com/r/jon-hedgerows/traefik-forward-auth/tags) and [github](https://github.com/jon-hedgerows/traefik-forward-auth/releases).
...
services:
  ...
  traefik-forward-auth:
    image: jon-hedgerows/traefik-forward-auth:2
    environment:
...
Please see the examples directory for a more complete [docker-compose.yml](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/swarm/docker-compose.yml) or [kubernetes/simple-separate-pod](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/kubernetes/simple-separate-pod/).

Also in the examples directory is [docker-compose-auth-host.yml](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/swarm/docker-compose-auth-host.yml) and [kubernetes/advanced-separate-pod](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/kubernetes/advanced-separate-pod/) which shows how to configure a central auth host, along with some other options.

#### Provider Setup

Below are some general notes on provider setup, specific instructions and examples for a number of providers can be found on the [Provider Setup](https://github.com/jon-hedgerows/traefik-forward-auth/wiki/Provider-Setup) wiki page.
...
Please see the [Provider Setup](https://github.com/jon-hedgerows/traefik-forward-auth/wiki/Provider-Setup) wiki page for examples.
...
This is an optional mode of operation that is useful when dealing with a large number of subdomains, it is activated by using the `auth-host` config option (see [this example docker-compose.yml](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/swarm/docker-compose-auth-host.yml) or [this kubernetes example](https://github.com/jon-hedgerows/traefik-forward-auth/tree/master/examples/traefik-v2/kubernetes/advanced-separate-pod)).
...
Please note: For Auth Host mode to work, you must ensure that requests to your auth-host are routed to the traefik-forward-auth container, as demonstrated with the service labels in the [docker-compose-auth.yml](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/swarm/docker-compose-auth-host.yml) example and the [ingressroute resource](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/examples/traefik-v2/kubernetes/advanced-separate-pod/traefik-forward-auth/ingress.yaml) in a kubernetes example.
...
[MIT](https://github.com/jon-hedgerows/traefik-forward-auth/blob/master/LICENSE.md)
